...
> **ローカルモデル版：** このリポジトリは、ローカルにデプロイされ、LANでホストされる
> 言語モデル向けのpi-webのメンテナンス版です。元のpi-webプロジェクトは、共有機能と
> 修正のためのアップストリームソースとして残っています。アップストリームの変更を定期的に
> 同期し、ローカルモデルのランタイムに対してレビューを行い、ローカルモデルの開発が並行して
> 進むようにこのラインを独立して公開しています。

この版のメンテナンスおよび開発ルールは、
[ローカルモデル版の開発](docs/dev/local-llm-development.md) に記載されています。
