...
> **本地模型版：** 本仓库是 pi-web 的一个维护版本，适用于本地部署和 LAN 托管的语言模型。
> 原始 pi-web 项目仍然是共享功能和修复的上游来源。我们定期同步上游更改，针对本地模型
> 运行时进行审核，并独立发布此分支，以便本地模型工作可以并行推进。

此版本的维护和开发规则记录在
[本地模型版开发](docs/dev/local-llm-development.md) 中。
