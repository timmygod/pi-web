...
此版本使用 pi 中已配置的服务商和模型；Local Mode 是一种运行时策略，而不是单独的模型安装程序或第二个 API 密钥界面。
在创建会话时选择模式，或在当前运行结束后更改模式：

| 模式 | 适用场景 | 行为 |
|------|-------------|----------|
| **Auto** | 您希望 pi-web 决定 | 尽可能从服务商元数据解析本地/LAN 端点；否则保持正常路径 |
| **Local** | 模型在此机器或您的 LAN 上运行 | 启用 65% 压缩边界、有界检查点、Force Compact 和受保护的自动恢复 |
| **Cloud** | 所选模型已托管并应遵循上游行为 | 将仅限本地的压缩和恢复策略排除在会话之外 |

手动选择 Local 或 Cloud 优先于自动检测，并在重新加载和重启后持续有效。正在运行的会话在其工作进程结束前拒绝模式更改，因此 UI 中显示的模式始终与实际使用的策略匹配。
